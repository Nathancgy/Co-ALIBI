...
def _co_alibi_fwd_kernel(
    ...
    P_raw_out, Sig_P_raw_out, Z_penalty_out, LSE_out,  # saved intermediates
    Out,  # output (attention result)
    # Strides
    ...
    for blk_idx_from_right in tl.static_range(KV_BLOCKS):
        ...
        if HAS_CAUSAL_MASK:
            ...
            p_raw_masked_for_sigma = p_raw_blk

        sig_blk = _sigmoid(p_raw_masked_for_sigma)
        if HAS_CAUSAL_MASK:
        ...
        # Softmax update (online, like flash-attention)
        ...
        tl.store(p_raw_out_ptr, p_raw_blk.to(P_raw_out.dtype.element_ty), mask=(offs_m[:, None] < seq_len_q) & key_valid_mask)
