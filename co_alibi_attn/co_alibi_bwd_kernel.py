...
def _co_alibi_bwd_kernel(
    ...
    max_row_idx = pid_m * BLOCK_M + BLOCK_M - 1
    ...
    for start_n in range(0, seq_len_kv, BLOCK_N_KV):
        ...
        proceed = (not HAS_CAUSAL_MASK) or (start_n <= max_row_idx)
        ...
        if proceed:
            ...
            z_penalty = row_sig_total - prefix_sig_sum_row - prefix_sig_local + sig_scaled
            prefix_sig_sum_row += tl.sum(sig_scaled, axis=1)[:, None]
            ...
            ds = tl.dot(do_blk, tl.trans(v_blk))
